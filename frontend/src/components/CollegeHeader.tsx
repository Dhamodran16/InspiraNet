...
const CollegeHeader: React.FC<CollegeHeaderProps> = ({ centered = false, showNav = true }) => {
  return (
    <header className="bg-white shadow-sm border-b border-gray-100 dark:border-gray-800 py-4">
      <div className={`container mx-auto px-4 flex items-center ${centered ? 'justify-center' : 'justify-between'}`}>
        {/* Logo and College Info */}
        <div className={`flex items-center space-x-4 ${centered ? 'text-center' : ''}`}>
          <img src="/logo.png" alt="Kongu Engineering College Logo" className="h-12 w-12 mx-auto" />
          <div className={centered ? 'hidden md:block' : ''}>
