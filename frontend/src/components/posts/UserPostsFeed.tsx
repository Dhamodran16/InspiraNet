...
const UserPostsFeed: React.FC<UserPostsFeedProps> = ({ userId, showDeleteButton = false }) => {
  const { user } = useAuth();
  ...
  // Load user's posts
  const loadUserPosts = useCallback(async (page = 1, append = false) => {
    if (!user || !user._id) {
      console.log('UserPostsFeed: No authenticated user, skipping post loading');
      return;
    }
    ...
    try {
      ...
      console.log('UserPostsFeed: Loading posts for user:', userId, 'page:', page);
      // Use dedicated backend endpoint for user posts to avoid client-side filtering
      ...
      if (response && response.posts) {
        ...
        if (append) {
          ...
          const newPosts = userPosts.filter(post => !existingIds.has(post._id));
        ...
        console.log('UserPostsFeed: Successfully loaded user posts:', userPosts.length);
      } else {
        console.warn('UserPostsFeed: Invalid response format:', response);
        setPosts([]);
        setHasMore(false);
      }
    } catch (error) {
      console.error('UserPostsFeed: Error loading user posts:', error);
  ...
  }, [user, userId]); // Removed posts dependency to prevent infinite loop

  // Initial load
  useEffect(() => {
    if (user && user._id && userId) {
      console.log('UserPostsFeed useEffect - loadUserPosts called for user:', userId);
      loadUserPosts(1, false);
    } else {
      console.log('UserPostsFeed useEffect - User not authenticated yet or no userId:', user, userId);
    }
  }, [user, userId]); // Removed loadUserPosts from dependencies to prevent infinite loop
  ...
  }, [isLoading, hasMore, currentPage, loadUserPosts]);
  ...
  return (
    <div className="space-y-4">
      {posts.map((post, index) => {
        ...
        const currentUserId = user?._id?.toString() || '';
        const isLiked = likesArray.some(likeId => likeId === currentUserId);
        ...
        console.log('🔍 Like check for post:', post._id, {
          ...
          exactMatch: likesArray.includes(currentUserId)
        });
