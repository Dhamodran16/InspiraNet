  ...
  try {
    ...
    if (userType === 'student') {
      ...
      if (currentYear) {
        ...
        // Indian academic year starts in July. A student in their Nth year in calendar year `now`
        // joined `n` years ago: e.g. 3rd-year student in 2026 joined in 2023 (2026 - 3 = 2023).
        joinYear = now - n;
        ...
        // Allow ±1 year tolerance as a safety buffer for students registering at the
        // very start/end of an academic year (July transition period).
        const derivedLast2 = joinYear % 100;
        const prevLast2 = (joinYear - 1) % 100;
        const nextLast2 = (joinYear + 1) % 100;
        if (emailYear !== derivedLast2 && emailYear !== prevLast2 && emailYear !== nextLast2) {
          console.log('❌ Email year mismatch - Email year:', emailYear, 'Derived join year:', joinYear, 'Last 2 digits:', derivedLast2);
          return res.status(400).json({
            success: false,
            error: `Email year (${emailYear}) does not match derived join year (${String(joinYear).slice(-2)})`,
            message: `The email year "${emailYear}" doesn't match the expected join year based on current year "${currentYear}". A ${currentYear}-year student in ${now} should have joined in ${joinYear} (email year: ${String(joinYear).slice(-2)}).`
          });
        }
        // Use the email year as the actual join year (more accurate)
        joinYear = 2000 + emailYear;
      } else if (batch) {
